...
class BruteForce:
    ...
    def addFailure(self):
        ...
        if ip not in self.database:
            ...
            self.logs.info(
                {
                    "message": "Increase IP failure counter.",
                    "ip": ip,
                    "failures": str(self.database[ip]["counter"]),
                }
            )
    ...
    def isIpBlocked(self) -> bool:
        ...
        if self.database[ip]["counter"] >= self.blockAfterFailures:
            ...
            if self.database[ip]["blockUntil"] < datetime.now():
                self.logs.warning(
                    {
                        "message": "Removing IP from the database, lucky guy, time expired.",
                        "ip": ip,
                    }
                )
                del self.database[ip]
