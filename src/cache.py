...
class Cache:
    ...
    def validateUser(self, username: str, password: str) -> bool:
        ...
        self.logs.info(
            {
                "message": "User not found in the cache for authentication.",
                "username": username,
            }
        )
        return False
    ...
    def validateGroups(self, username: str, groups: list):
        ...
        if username in self.cache:
            ...
            self.logs.info(
                {
                    "message": "Validating groups from cache.",
                    "username": username,
                    "groups": ",".join(groups),
            ...
            if self.groupConditional == "or":
                if len(matchedGroups) > 0:
                    self.logs.info(
                        {
                            "message": "At least one group from cache is valid for the user.",
                            "username": username,
            ...
            # Conditional AND, true if all the groups match
                ...
                if len(groups) == len(matchesByGroup):
                    self.logs.info(
                        {
                            "message": "All groups from cache are valid for the user.",
                            "username": username,
                            "matchedGroups": ",".join(matchedGroups),
                            "groups": ",".join(groups),
                            "conditional": self.groupConditional,
            ...
            self.logs.warning(
                {
                    "message": "Invalid groups from cache.",
                    "username": username,
                    "conditional": self.groupConditional,
                }
            )
        ...
        self.logs.info(
            {
                "message": "User not found in the cache for validate groups.",
                "username": username,
            }
        )
        return False, []
